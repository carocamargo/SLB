...
for iname,name in enumerate(datasets):
    ds=xr.open_dataset('/Volumes/LaCie_NIOZ/data/barystatic/use/'+
...
#%%
da=xr.Dataset(data_vars={'manometric_sl':(('names','time','lat','lon'),OM_ts),
                         # 'fingerprint':(('names','lat','lon'),fing),
                         # 'dynamic_sl':(('names','lat','lon'),dyn_SL)
                         },
                         coords={'names':datasets,
                                 'time':ds.time,
                                 'periods':['2003-2016','2005-2015'],
...
for j,period in enumerate(periods):
    t0,t1 = period
    to=str(t0)+'-01-01';ti=str(t1)+'-12-31'
    ...
    for iname,name in enumerate(datasets):
        ...
        fing[iname,j,:,:] = np.array(df['{}_trend_tot'.format(name)]).reshape(dimlat,dimlon)
...
da.attrs['metadata']='Dynamic SL computed by removing the fingerprints (barystatic SL) from the ocean mass signal of GRACE'
