...
names=[name for name in np.array(ds.name) if not name=='ARMOR3D' and not name=='soda_342']
ds = ds.sel(name=names)
min_lat = -66
max_lat = 66
mask_lat = (ds.lat >= min_lat) & (ds.lat <= max_lat)
# da = ds.where(mask_lat, drop=True) # remove anything out of our mask # dimension of latitude is 132
da = ds.where(mask_lat) # add nans on anything out of the mask # dimension of latitude is 180
...
#%% make dataset
xa=xr.Dataset(data_vars={'ssh_anom':(('time','lat','lon','name'),dyn_sl),
...
xd=xr.Dataset(data_vars={'dyn_sl':(('time','lat','lon','name'),dyn_sl[:,:,:,0:len(da.name)]),
