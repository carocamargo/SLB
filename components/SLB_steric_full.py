...
steric_up = np.array(ds.sla_ens)
lat=np.array(ds.lat)
lon=np.array(ds.lon)
steric_full = np.array((steric_up) +  (deep)/1000) # meters
tr,_ = sl.get_reg_trend(tdec, steric_full*1000, 
                        lat, lon)
plt.pcolor(tr,vmin=-5,vmax=5,cmap='RdBu_r');plt.colorbar()
plt.title(str(np.nanmean(tr)))

#%% test deep ts
tr,_ = sl.get_reg_trend(tdec, deep, 
                        lat, lon)
plt.pcolor(tr,vmin=-5,vmax=5,cmap='RdBu_r');plt.colorbar()
plt.title('Trend from time series\n'+str(np.nanmean(tr)))
plt.show()

plt.pcolor(ds2.steric,vmin=-5,vmax=5,cmap='RdBu_r');plt.colorbar()
plt.title('Original trend\n'+str(np.nanmean(ds2.steric)))
plt.show()

plt.pcolor(tr - np.array(ds2.steric),vmin=-0.1,vmax=0.1,
           cmap='RdBu_r');plt.colorbar()
...
tr,_ = sl.get_reg_trend(tdec, steric_full[-1]*1000, 
                        lat, lon)
plt.pcolor(tr,vmin=-5,vmax=5,cmap='RdBu_r');plt.colorbar()
plt.title(str(np.nanmean(tr)))
#%% make dataset
ds=xr.Dataset(data_vars={'steric_up':(('names','time','lat','lon'),steric_up),
...
ds.attrs['deep_steric']='Deep steric from Purkey & Johnson updated'
