...
for period in periods:
    ...
    dimtime = len(time)
    time = get_dectime(np.array(pd.date_range(start='1/1/{}'.format(t0), periods=dimtime, freq='M')))
    ...
    for iname, name in enumerate(names):
        ...
        for inm, n in enumerate(NM):
            ...
            for ilon in range(len(lon)):
                if np.any(np.isfinite(data2[:,ilon])): # check if we have data:
                    ...
                    # Create a .ctl file:
                    hec.create_estimatetrend_ctl_file(name,n,sp=sp,GGM_1mphi = 6.9e-07,LikelihoodMethod='FullCov')
            ...
            # save by NM
            dn=xr.Dataset(data_vars={'trend':(('lat','lon'),tr[inm,:].reshape(dimlat,dimlon)),
                                  ...
                                  'N':(('lat','lon'),N[inm,:].reshape(dimlat,dimlon)),
                                    ...
                                    coords={'nm':NM[inm],
                                            ...
                                            'lat':ds.lat,
                                            'lon':ds.lon})
            dn.attrs['metadata']=' sea-level trends in mm/y from {} to {}, obtained with Hector'.format(
                t0,t1)
        ...
        dx=xr.Dataset(data_vars={'trend':(('nm','lat','lon'),tr.reshape(len(NM),dimlat,dimlon)),
                              ...
                              'N':(('nm','lat','lon'),N.reshape(len(NM),dimlat,dimlon)),
                                ...
                                coords={'nm':NM,
                                        ...
                                        'lat':ds.lat,
                                        'lon':ds.lon})
        dx.attrs['metadata']=' sea-level trends in mm/y from {} to {}, obtained with Hector'.format(
            t0,t1)
        path_to_save ='{}/hector/{}-{}/'.format(pwd,t0,t1)
        # os.system(' cd {}'.format(path_to_save))
