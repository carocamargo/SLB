...
ds = xr.Dataset(data_vars={'steric_up':(('names','time','lat','lon'),data_up),
...
def get_time(time):
    t = datetime.utcfromtimestamp((time- np.datetime64('1970-01-01T00:00:00Z')) / np.timedelta64(1, 's')) 
         # for t in time]
...
#% %
ds=xr.Dataset(data_vars={'ssh':(('names','time','lat','lon'),ssh),
...
da.attrs['ensemble']='ENS, both of dynamic and steric is the ensemble of the 5 reanalysis'
...
ds=xr.Dataset(data_vars={'ens_dyn_v1':(('time','lat','lon'),ens_dyn),
...
ds.attrs=da.attrs
ds.attrs['v1'] = 'First we compute DSL and SDSL from each reanalysis, then take the mean (ensemble)'
ds.attrs['v2'] = 'First we take the mean of SSH and SSL from the 5 reanlysis, then compute DSL and SDSL'
