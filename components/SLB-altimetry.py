...
da=xr.Dataset(data_vars={'sla':(('time','lat','lon'),sla)},
                  coords={'time':time,
...
for year in years:
    flist = sorted([file for file in os.listdir(path+'5day/') if file.endswith('.nc') 
...
for file in flist:
    os.system('cdo -L remapbil,r360x180 ' + 
                      str(path_save+file) + ' ' + str(path_save+'regrid/'+file))
