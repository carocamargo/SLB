...
for iname, name in enumerate(names):
    ...
    for ireg,reg in enumerate(np.array(regs)):
        ...
        for itime in range(timedim):
            ...
            regional_mass_change_EWH[ireg,itime,:,:] = sle.height_to_EWH(glb_to_reg(
                                    np.array(data[itime]),mask)).reshape(180,360)
    ...
    da=xr.Dataset(data_vars={
                            # 'SLF_ASL':(('reg','time','lat','lon'),regional_SLF_rsl),
                              ...
                              'mass_change_EWH':(('reg','time','lat','lon'),regional_mass_change_EWH),
    ...
    for itime in range(timedim):
        # run SLE
        X=np.array(ds2.lon); Y=np.array(-ds2.lat)
        ...
        regional_SLF_rsl[itime,:,:], _ = sle.get_output(var='rsl',
...
for iname, name in enumerate(names):
    ...
    # make regional dataset:
    da=xr.Dataset(data_vars={'SLF_ASL':(('time','lat','lon'),regional_SLF_rsl),
...
for file in flist:
    ...
    if name.split('_')[-1]=='IMB':
        ...
        dt = [np.array('{}-{}'.format(y,str(m).zfill(2)),dtype='datetime64') for y,m in zip(y,m)]
        da['time'] = dt
...
for ic, combo in enumerate(combos):
    ...
    var = [key for key in da.variables if key.split('_')[0]=='SLF' and key.split('_')[1]=='ASL']
    ...
    var = [key for key in da.variables if key.split('_')[0]=='SLF' and key.split('_')[1]=='RSL']
    var = [var for var in var if var.split('_')[-1] in datasets]
...
for ic in range(len(combos)):
    ...
    data_combo2[ic] = data_combo2[ic] - np.nanmean(data_combo2[ic,144:276,:,:],axis=0)
    data_combo3[ic] = data_combo3[ic] - np.nanmean(data_combo3[ic,144:276,:,:],axis=0)
