...
dx=xr.Dataset(data_vars={'unc':(('names','period','lat','lon'),unc),
                          'trend_up':(('names','period','lat','lon'),trend),
                        },
                          coords={'names':np.array(ds.fname),
                                  'period':periods,
                                  'lat':ds.lat,
                                  'lon':ds.lon,
                                  })
...
for ip in range(len(periods)):
    for iname in range(len(ds.fname)):
        trend[iname,ip,:,:] = np.array(trend[iname,ip,:,:] + deep)
        # unc[iname,ip,:,:] = np.array(unc[iname,ip,:,:] + deep)

plt.pcolor(trend[iname,ip,:,:]-np.array(ds['trend_{}'.format(y0)][iname]))        
#%%
dx['trend_full'] =(['names','periods','lat','lon'],trend)
...
#%%
dx.attrs['Summary']='Full depth Best Steric trend and uncertainty according to the noise-model analysis \
...
dx.attrs['Description']='Linear trends from 1993-2017 and 2005-2015 (full years, jan-dec)'
dx.attrs['script']='SLB-steric_trends.py'
dx.attrs['author']='Carolina Camargo (carolina.camargo@nioz.nl)'
dx.attrs['journal_reference']=' Camargo et al., 2020, JGR:Oceans,  https://doi.org/10.1029/2020JC016551'
dx.attrs['dataset_reference']='https://doi.org/10.4121/uuid:b6e5e4bc‐d382‐4b51‐837b‐c5cde4980bf3'
