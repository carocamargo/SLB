...
def sel_best_NM(IC,field,field2):
    ...
    return score.reshape(dimlat,dimlon),best_field.reshape(dimlat,dimlon), best_field2.reshape(dimlat,dimlon), 

#%%
def add_attrs(ds,variables=['lat','lon'],latname='lat',lonname='lon',depthname='depth'):
    if 'lat' in variables:
        ds[latname].attrs = {'axis': 'Y',
         'long_name': 'latitude',
         'standard_name': 'latitude',
         'unit_long': 'degrees north',
         'units': 'degrees_north'}
    if 'lon' in variables:
        ds[lonname].attrs = {'axis': 'X',
         'long_name': 'longitude',
...
for ip,period in enumerate(periods):
    ...
    if len(nm) != 8: print('Missing Noise Model !!!!!')
    
    #% % plottrend an unc for all noise models 
    ds=xr.open_dataset(path+'ALT_ENS.nc')
    ds = ds.where((ds.lat>=-66) & (ds.lat<=66),np.nan)
    if plot:
        ds.trend.plot(col='nm',col_wrap=4,
                      vmin=cmin,vmax=cmax,cmap=cmap)
        plt.show()
        ds.unc.plot(col='nm',col_wrap=4,
                      vmin=0,vmax=cmax,cmap=cmap2)
        plt.show()
    ...
    for i,ind in enumerate(inds):
        score,best_trend,best_unc = sel_best_NM(np.array(ds[ind]),
                                                np.array(ds.trend),np.array(ds.unc))
        #% %
        if plot:
            cmap=cm.cm.balance
            clim=5
            ...
            plt.pcolor( (ds.sel(nm='AR1').trend /1000) - (best_trend/1000),
                       vmin=-1,vmax=1,cmap=cm.cm.curl)
            cbar = plt.colorbar()
...
#% % add data to dataset
    if ip==0:
        da = xr.Dataset(data_vars={'NM_score':(('periods','ICs','lat','lon'),scores),
