...
depth=0
time=0
ds[var][time,depth,:,:].plot();plt.show()
...
da=xr.Dataset(data_vars={'data':(('lat','lon'),data)},
                 coords={'lat':lat,
                         'lon':lon
                         })
...
da=xr.Dataset(data_vars={'data':(('lat','lon'),data-glb)},
                 coords={'lat':lat,
                         'lon':lon
                         })
da.data.plot();plt.show()
#%%
dataset_name='ARMOR3D'
path='/Volumes/SLB/raw_data/steric/ARMOR3D/Data/'
flist=sl.get_filelist(path,ext='*.nc')
...
for dataset in datasets:
    ...
    #% %
    da=xr.Dataset(data_vars={'dyn_sl':(('time','lat','lon'),dyn_sl),
    ...
    da.attrs['metadata']='Dynamic sea-level height, calculated from sea-surface height by uniformly subtracting its time-varying global mean.'
...
command = 'cdo -f nc -sellonlatbox,{},{},{},{} -random,r360x180 {}template.nc'.format(left_lon,right_lon,lower_lat,upper_lat,path_save+'/regrid/')
os.system(command)
#%% regrid
for f in flist:
    ...
    command = 'cdo -remapbil,{}template.nc {} {}'.format(path_save+'regrid/',infile,ofile)
    os.system(command)
    
#%% correct time 
filepath=path_save+'regrid/'
flist=[file for file in os.listdir(filepath) if not file.startswith('temp') 
...
for i,file in enumerate(datasets):
    ...
    ds['time']=pd.date_range(start='{}-01-01'.format(start[i]), 
                         end='{}-01-01'.format(end[i]+1), 
                         freq='M')
...
time = np.array(pd.date_range(start='{}-01-01'.format(start[0]), 
                         end='{}-01-01'.format(end[0]+1), 
                         freq='M'))
...
for i,file in enumerate(datasets):
    ...
    #% %
da=xr.Dataset(data_vars={'ssh_anom':(('time','lat','lon','name'),dyn_sl),
...
da.attrs['metadata']='SSH from reanalysis, and SSH anomaly computed by removing the time-varying global mean (gmsl variable)'
da.attrs['script']='dyn_sl.py'
