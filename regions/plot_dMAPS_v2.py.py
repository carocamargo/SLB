...
#%%
paths = ['/Volumes/LaCie_NIOZ/reg/world/dmaps/world_v2/',
         ]
geofiles =['MSLA_CMEMS_glb_merged_1993-2019_detrend_deseason_300kmfilter_res1deg.nc',
           ]
i=0
for geofile,path in zip(geofiles,paths):
    ...
    dmaps.plot_nmi_matrix(nmi, np.arange(4,25),fname=path+'plot/nmi_matrix_res{}.png'.format(i)) 
    for k in range(4,25):
        print(k)
        dmaps.plot_dMaps_output(geofile = path+geofile,
...
for i,k in enumerate(kline):
    ...
    perc_clustered_pixels[i] = len(domains[domains==1])/len(domains[np.isfinite(domains)])
...
# xline = np.sin(zline)
# yline = np.cos(zline)
ax.plot3D(kline[number_clusters<120],  perc_nans[number_clusters<120],number_clusters[number_clusters<120], 'gray')
sc = ax.scatter3D(kline[number_clusters<120],  perc_nans[number_clusters<120], number_clusters[number_clusters<120],c=perc_nans[number_clusters<120],cmap='plasma',s=150)
# plt.xlabel('k')
# plt.ylabel('number clusters')
# plt.zlabel('% not clustered pixels')
ax.set_xlabel('K')
...
#%% 
# import pandas as pd
df = pd.DataFrame({'k':kline,'n_clusters':number_clusters,'perc_not_cluster':perc_nans,
