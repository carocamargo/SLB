...
print(ds.title)
print('Time resolution : {} from {} to {} total of {} timesteps'.format(ds.time_coverage_resolution, 
                                                     np.array(ds.time[0]), np.array(ds.time[-1]), len(ds.time)))
print('Spatial resolutions: {} degrees \n longitude {} to {} \n latitude {} to {} '.format(ds.geospatial_lon_resolution,
                                                                                     np.array(ds.longitude.min()),np.array(ds.longitude.max()),
                                                                                     np.array(ds.latitude.min()),np.array(ds.latitude.max()),
...
for file in files:
    ...
    da=xr.Dataset(data_vars={var_name:((time_name,lat_name,lon_name),data_filtered)},
                      coords={time_name:ds[time_name],
                              lon_name:ds[lon_name],
                              lat_name:ds[lat_name],
                              })
    da.attrs=dataset.attrs
    da.attrs['post-processing']='SLA detrended, (deseasonalized), and  300km filtered.'
...
command = 'cdo -f nc -sellonlatbox,{},{},{},{} -random,r360x180 {}template.nc'.format(left_lon,right_lon,lower_lat,upper_lat,path_working)
os.system(command)
# b. get list of the 300km filters
flist=[filename for filename in os.listdir(path_working) if filename.endswith('300kmfilter.nc') and filename.startswith('MSLA')]
...
files = ['MSLA_CMEMS_glb_merged_1993-2019_detrend_deseason_300kmfilter_res1deg.nc',
