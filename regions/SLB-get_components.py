...
t1=2017
infile = ofile
outfile = path_save + ofile.split('/')[-1].split(t0_old)[0]+'{}-{}'.format(t0,t1) + ofile.split('/')[-1].split(t1_old)[-1]
command = 'cdo selyear,'+str(t0)+'/'+str(t1)+' '+str(infile)+' '+str(outfile)
os.system(command)
...
ds[var][0,:,:].plot();plt.show()
ds2=ds.where((ds.lat<np.max(da.lat)) & (ds.lat>np.min(da.lat)),np.nan)
ds2[var][0,:,:].plot();plt.show()

#% % steric trends
ds=xr.open_dataset(path_data+file2)
print(ds)
vars = list(ds.keys())
da2= xr.Dataset(data_vars={var:(('lat','lon'),ds[var]/1000) for var in vars},
                          coords={'lat':da.lat,
                                  'lon':da.lon})
for var in vars:
...
dsteric.attrs['description'] = 'Steric SL trend and time series for 1993-2017 and 2005-2015 for ENS'
...
ds2.SLF[0,0,:,:].plot();plt.show()
#% % check dimensions
np.all(ds.lat==ds2.lat)
np.all(ds.lon==ds2.lon)
print(ds2.name)
ds2['name'] = ['AIS_IMB', 'AIS_R19', 'GLA_ZMP', 'GLA_WGP', 'AIS_CSR',
       'GIS_CSR', 'LWS_CSR', 'GLWS_CSR', 'TCWS_CSR', 'AIS_JPL',
       'GIS_JPL', 'LWS_JPL', 'GLWS_JPL', 'TCWS_JPL', 'GIS_IMB', 'GIS_M19',
       'LWS_GWB', 'LWS_WGP']
print(ds2.name)
#% % 
...
for title,combo in zip(reconstr,combos):
        ...
        #% %
        names=[name for name in np.array(ds2.name) for comb in combo if name.split('_')[1]==comb ]
        rec_SLF[i,:,:,:] = np.array(ds2.SLF.sel(name=['AIS_IMB','GIS_IMB','GLA_WGP','LWS_WGP'])).sum(axis=0)
...
ind = np.arange(0,180)
ind = ind[::-1]
dbary = xr.Dataset(data_vars={ 'SLF':(('name','time','lat','lon'),ds2['SLF']/1000),
...
dbary.trend_1993[0,:,:].plot();plt.show()
dbary['lat']=-dsla.lat
dbary.trend_1993[0,:,:].plot();plt.show()

dbary.attrs['description'] = 'Barystatic SL trend and time series for 1993-2016 and 2003-2016'
dbary.attrs['units'] = 'm'
...
for n in np.array(dbary.name):
    dbary.SLF.sel(name=n).mean(dim=['lat','lon']).plot();plt.show()
#%%
ds = dbary.rec_SLF.sel(reconstruction=r)
tdec,tdec0 = sl.get_dec_time(np.array(ds.time))
trend ,_= sl.get_reg_trend(tdec,np.array(ds.data*1000),
                            np.array(ds.lat),np.array(ds.lon))
plt.pcolor(ds.lon,ds.lat,trend,vmin=-6,vmax=6,cmap='RdBu_r');plt.colorbar()
